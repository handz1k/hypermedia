...
const timeToFirstMsg  = new Trend('spa_time_to_first_msg_ms',  true);
const msgIntervalMs   = new Trend('spa_msg_interval_ms',       true);
const msgBytesRecv    = new Trend('spa_msg_bytes_received',    false);
...
export const options = {
  ...
  thresholds: {
    'spa_time_to_first_msg_ms': ['p(95)<500'],
    'spa_msg_interval_ms':      ['p(95)<' + (UPDATE_INTERVAL_MS * 3)],
    'ws_connecting':            ['p(95)<100'],
  },
};

export default function () {
  const res = ws.connect(TARGET_URL, { headers: { 'Sec-WebSocket-Protocol': 'spa-ticker' } }, function (socket) {
    activeConns.add(1);
